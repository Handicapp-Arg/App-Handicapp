...
export default function CaballosPage() {
  ...
  const handleImageUpload = async (horseId: string, files: File[]) => {
    if (files.length > 0) {
      await uploadImage.mutateAsync({ id: horseId, file: files[0] });
    }
    setEditImageFiles([]);
  };
      ...
      {showForm && (
        ...
        >
          <div className="space-y-3">
            ...
            <button
              type="submit"
              disabled={createHorse.isPending || uploadImage.isPending}
                ...
                <div className="p-4 space-y-3">
                  <ImagePicker
                    ...
                    onChange={(files) => {
                      ...
                      if (files.length > 0) handleImageUpload(horse.id, files);
                    }}
                    single
                    label="Foto del caballo"
                    existingUrl={horse.image_url}
                    onRemoveExisting={() => handleRemoveImage(horse.id)}
                  />
                  <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => { setEditingId(null); setEditImageFiles([]); }}
